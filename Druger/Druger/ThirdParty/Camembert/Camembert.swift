...
//        println(pathDatabase)
    ...
    func getObjectsWithQuery(query :String, table :String) -> [AnyObject]! {
        ...
        while (sqlite3_step(ptrRequest) == SQLITE_ROW) {
            var aaa: AnyObject! = camembertCreateObject(table)
            var currentObject: CamembertModel = aaa as! CamembertModel
            
            currentObject.setId(Int(sqlite3_column_int(ptrRequest, 0)))
